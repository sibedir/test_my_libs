...
namespace sib {
    ...
    private:
        ...
        using construction_base_type_from = construct_from_tooneof_select<AnyT, Ts...>;

        template <typename AnyT>
        using construction_type_from = TWrapper<construction_base_type_from<AnyT>>;

        template <typename AnyT>
        static constexpr bool noexcept_constr = noexcept(TWrapper<construct_from_tooneof_select<AnyT, Ts...>>(std::declval<AnyT>()));
    ...
    public:
        ...
        template <typename... AnyTs>
            requires(
                    (is_constructible_from_tooneof_v<AnyTs, Ts...> and ...)
                and no_duplicates_v<construction_base_type_from<AnyTs>...>
            )
        ...
        template <typename AnyT>
        constexpr assign_from_tooneof_result<AnyT, Ts...> operator= (AnyT&& other)
            noexcept(noexcept(TWrapper<assign_from_tooneof_select<AnyT, Ts...>>::operator=(std::declval<AnyT>())))
        { return TWrapper<assign_from_tooneof_select<AnyT, Ts...>>::operator=(std::forward<AnyT>(other)); }
